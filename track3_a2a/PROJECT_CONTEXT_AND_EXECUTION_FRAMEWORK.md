...
The v1.6.2 CUDA pre-integration audit refined that diagnosis: retrieved reference displacement was only `1.5e-6 nm` and constraint projection only `1.2e-6 nm`, but the restraint term was still `3.736e8 kJ/mol`. Version 1.6.3 tested explicit scalar reference components; its corrected-only CUDA probe reproduced the same restraint energy and became non-finite at step 208. This falsified the parameter-container hypothesis and showed that retrieved coordinates did not expose the periodic image used by the CUDA force evaluation. Version 1.6.4 therefore makes the positional restraint periodic-image invariant with OpenMM `periodicdistance`. This implementation correction preserves every scientific and numerical setting. The failed attempts remain recorded and excluded; production and Tier B remain locked.

The v1.6.4 corrected-only mixed-precision CUDA validation subsequently completed 10,000 steps with a restraint energy of `2.50e-9 kJ/mol`, compared with `3.736e8 kJ/mol` before the correction. It had no duplicate constraints and remained finite. The numerical preflight is therefore accepted and the first full staged-equilibration job is authorized. This validation is not itself an equilibration result and does not unlock production or Tier B.

All three full v1.6.4 5NM4-ZMA equilibration replicas subsequently completed 2.14 ns and passed. Seeds 20260914, 20260915, and 20260916 ended at `309.0 K`, `309.75 K`, and `309.80 K`; native-contact fractions were `0.8533`, `0.8933`, and `0.8267`; last-half volume CVs were `0.00170`, `0.00172`, and `0.00168`; and all monitored energy, temperature, and volume values were finite. Seed 20260916 passes the frozen contact threshold by `0.0267` and is retained without post hoc exclusion. These count as three of six required equilibration replicas; production and Tier B remain locked pending all three 5G53 replicas and the complete aggregate gate.
