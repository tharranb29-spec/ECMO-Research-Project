...
The v1.6.2 CUDA pre-integration audit refined that diagnosis: reference displacement was only `1.5e-6 nm` and constraint projection only `1.2e-6 nm`, but the restraint term was still `3.736e8 kJ/mol`. Thus periodic imaging was no longer responsible. The platform-sensitive fault was passing a `Vec3` object as a three-value per-particle parameter vector. Version 1.6.3 converts each reference component to an explicit scalar float before force construction. This compatibility-only correction preserves every scientific and numerical setting. The failed attempts remain recorded and excluded.
